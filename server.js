  ...
  fs.createReadStream(req.file.path)
    ...
    .on('data', (data) => {
      results.push({
        ...
        lastActivityDate: data['Last Activity Date'] ? new Date(data['Last Activity Date']) : null,
...
async function sendReminderEmails() {
  ...
  for (const deal of deals) {
    ...
    const msg = {
      ...
      subject: `Reminder: Follow up on deal ${deal.dealName}`,
      text: `Please follow up on deal "${deal.dealName}" (Owner: ${deal.dealOwner}, Stage: ${deal.dealStage}). Last activity was on ${deal.lastActivityDate.toDateString()}.`,
    };

    try {
      await sgMail.send(msg);
      console.log(`Email sent to ${email} for deal ${deal.dealName}`);
    } catch (err) {
      console.error(`Error sending email for deal ${deal.dealName}:`, err);
    }
  }
}
